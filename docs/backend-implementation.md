...
6. **다운로드 감사 scope:** leaf subtype→Series.ManagementAuthorityID와 저장된 관리 상위 관계에서 scope를 구성한다. 현재 MVP도 scope를 비워 저장하지 않는다. 인증서의 역사적 발급 체인은 2번 경로로 별도로 구하고 관리 관계와 혼용하지 않는다. 전환으로 여러 CA가 관련된 작업은 기존 복수 scope 감사 규칙을 적용한다. 성공/실패/복구 감사에서 동일한 관계 기반 scope를 사용하며 caller가 넘긴 AuthorityID를 신뢰하지 않는다.
